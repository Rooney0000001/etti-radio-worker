  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === '/api/posts' && method === 'POST') {
        const body = await request.json();
        const { title, content, imageUrl, password } = body;
        ...
        ).bind(title.trim(), content.trim(), (imageUrl || '').trim()).run();
      ...
      if (postIdMatch && method === 'PUT') {
        ...
        ).bind(body.title.trim(), body.content.trim(), (body.imageUrl || '').trim(), postIdMatch[1]).run();
        return json({ success: true });
      }
